...
import { ClockIcon, StarIcon, CheckCircleIcon, SearchIcon, CalendarIcon, Loader2 } from "lucide-react"
...
function ServicesPage() {
  ...
  const handleBookSubmit = async (data: FormValues) => {
    ...
    const serviceId =
      services
        .find((service) => service.name.toLowerCase().replace(" ", "-") === data.service)
        ?.serviceId?.toString() || ""
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
            <div className="flex flex-col items-center">
              ...
              <p className="text-muted-foreground text-center">We deliver nothing but the best in every service.</p>
            </div>
            <div className="flex flex-col items-center">
              <ClockIcon className="w-12 h-12 text-primary mb-4" />
              <h3 className="text-xl font-semibold mb-2">Timely Service</h3>
              <p className="text-muted-foreground text-center">Punctuality is our promise to you, every time.</p>
            </div>
            <div className="flex flex-col items-center">
              <CheckCircleIcon className="w-12 h-12 text-primary mb-4" />
              <h3 className="text-xl font-semibold mb-2">Customer Satisfaction</h3>
              <p className="text-muted-foreground text-center">Your happiness is our top priority.</p>
            </div>
          </div>
        </motion.div>
                ...
                >
                  <Card className="h-full flex flex-col hover:shadow-lg transition-shadow duration-300">
                    ...
                    <CardFooter className="mt-auto flex justify-between items-center">
                      <Button variant="outline" onClick={() => openBookDialog(service.name)}>
                        Book Now
                      </Button>
                      <Badge variant="secondary">${service.baseFee.toFixed(2)}</Badge>
                    </CardFooter>
                  </Card>
                </motion.div>
              ))}
          ...
          <Dialog open={isBookDialogOpen} onOpenChange={setIsBookDialogOpen}>
            <DialogContent className="sm:max-w-[425px] md:max-w-[600px] lg:max-w-[800px] z-dialog">
              <DialogHeader>
                ...
                <DialogDescription>
                  Fill out the form below to book a service. We'll confirm your booking shortly.
                </DialogDescription>
              </DialogHeader>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(handleBookSubmit)} className="space-y-3">
                  <FormField
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          ...
                          <SelectContent>
                            {services.map((service) => (
                              <SelectItem key={service.serviceId} value={service.name.toLowerCase().replace(" ", "-")}>
                                {service.name}
                              </SelectItem>
                            ))}
                          </SelectContent>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea placeholder="Additional details (optional)" className="resize-none" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
